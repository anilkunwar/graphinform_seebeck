...
# -----------------------------
# Advanced NER for Seebeck values
# -----------------------------
@Language.component("seebeck_ner")
def seebeck_ner(doc):
    seebeck_pattern = r'\b([-+]?\d+(?:\.\d+)?)\s*(?:μV/K|µV/K|μV·K⁻¹|µV·K⁻¹|microvolt per kelvin)\b'
    spans = []
    for match in re.finditer(seebeck_pattern, doc.text):
        value = match.group(1)
        if -500 <= float(value) <= 500:
...
def link_formula_to_seebeck(doc):
    ...
    for f in formulas:
        ...
        if nearest_seebeck:
            pairs.append({
                "Formula": f.text,
                "Seebeck_Value": nearest_seebeck.text,  # Parse to float in extraction
            })
    return pairs
        ...
        for i, row in df.iterrows():
            ...
            for chunk_idx, chunk in enumerate(chunks):
                ...
                for pair in linked_pairs:
                    try:
                        seebeck_value = float(pair["Seebeck_Value"])
...
def plot_seebeck_values(df, top_n=20, year_range=None):
    ...
    material_counts = df.groupby("material")["seebeck"].agg(['mean', 'count']).reset_index()
    top_materials = material_counts.groupby("material")["mean"].sum().nlargest(top_n).index
    ...
    fig_bar = px.bar(
        ...
        x="material", 
        y="mean", 
        title=f"Top {top_n} Materials by Seebeck Coefficient",
        labels={"material": "Formula", "mean": "Average Seebeck Coefficient (μV/K)"},
        color_discrete_map={"seebeck": "#636EFA"}
    )
    ...
    if 'year' in df.columns and df["year"].notna().any():
        ...
        fig_timeline = px.line(
            ...
            x="year",
            y="seebeck",
            title="Trend of Seebeck Coefficients Over Time",
            labels={"year": "Year", "seebeck": "Average Seebeck Coefficient (μV/K)"},
            color_discrete_map={"seebeck": "#636EFA"}
        )
    ...
    # Co-occurrence heatmap (for materials)
    ...
    if 'year' in df.columns:
        sunburst_data = df.groupby(['year', 'material'])["seebeck"].mean().reset_index()
        fig_sunburst = px.sunburst(
            sunburst_data,
            path=['year', 'material'],
            values='seebeck',
            title="Hierarchical Distribution of Seebeck Coefficients",
            color='seebeck',
            labels={"year": "Year", "material": "Formula", "seebeck": "Seebeck Coefficient (μV/K)"}
        )
        fig_sunburst.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
    else:
        sunburst_data = df.groupby(['material'])["seebeck"].mean().reset_index()
        fig_sunburst = px.sunburst(
            sunburst_data,
            path=['material'],
            values='seebeck',
            title="Hierarchical Distribution of Seebeck Coefficients (No Year Data)",
            color='seebeck',
            labels={"material": "Formula", "seebeck": "Seebeck Coefficient (μV/K)"}
        )
        fig_sunburst.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
