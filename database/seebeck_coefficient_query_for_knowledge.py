...
def download_pdf_and_extract(pdf_url, paper_id, paper_metadata):
    ...
    try:
        ...
        response.raise_for_status()  # FIXED: Changed from raise_forcelist() to raise_for_status()
...
def create_pdf_zip(pdf_paths):
    ...
    zip_path = os.path.join(DB_DIR, "seebeck_pdfs.zip")  # FIXED: Changed to seebeck_pdfs.zip
    try:
...
if st.session_state.search_results and st.session_state.relevant_papers:
    ...
    # Restore download buttons - FIXED: Ensure all download buttons are properly displayed
    ...
    if "CSV" in output_formats:
        ...
        st.download_button(
            ...
            data=csv,
            file_name="seebeck_papers.csv",  # FIXED: Changed to seebeck_papers.csv
            mime="text/csv",
            key=f"csv_download_{time.time()}"  # FIXED: Added timestamp for unique key
        )
    ...
    if "JSON" in output_formats:
        ...
        st.download_button(
            ...
            data=json_data,
            file_name="seebeck_papers.json",  # FIXED: Changed to seebeck_papers.json
            mime="application/json",
            key=f"json_download_{time.time()}"  # FIXED: Added timestamp for unique key
        )
    
    # Display individual PDF links - FIXED: Ensure PDF download buttons work
        ...
        for idx, pdf_path in enumerate(pdf_paths):
            ...
            if file_data:
                st.download_button(
                    ...
                    key=f"pdf_download_{idx}_{time.time()}"  # FIXED: Added timestamp for unique key
                )
    
        # ZIP download - FIXED: Ensure ZIP download button works
        ...
        if zip_path and os.path.exists(zip_path):
            ...
            if file_data:
                st.download_button(
                    ...
                    data=file_data,
                    file_name="seebeck_pdfs.zip",  # FIXED: Changed to seebeck_pdfs.zip
                    mime="application/zip",
                    key=f"zip_download_{time.time()}"  # FIXED: Added timestamp for unique key
                )
    
    # Database file downloads - FIXED: Ensure database download buttons work
    ...
    if os.path.exists(METADATA_DB_FILE):
        ...
        if file_data:
            st.download_button(
                ...
                key=f"metadata_db_download_{time.time()}"  # FIXED: Added timestamp for unique key
            )
    else:
    ...
    if os.path.exists(UNIVERSE_DB_FILE):
        ...
        if file_data:
            st.download_button(
                ...
                mime="application/octet-stream",
                key=f"universe_db_download_{time.time()}"  # FIXED: Added timestamp for unique key
            )
    else:
        ...
        try:
            ...
            if not papers:
                ...
                if not relevant_papers:
                    ...
                    # Export data - FIXED: Ensure all download buttons work with proper file names
                    if "SQLite (.db)" in output_formats:
                        sqlite_status = save_to_sqlite(df.drop(columns=["abstract_highlighted"]), [])
                        st.info(sqlite_status)
